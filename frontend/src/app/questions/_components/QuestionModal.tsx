...
export default function QuestionModal({
  ...
  let [successText, setSuccessText] = useState(
    question.is_answered ? "Already answered!" : ""
  );
  ...
  return (
    <div
      className={`${s.container} ${closing ? s.closing : ""}`}
      onClick={handleClose}
    >
          ...
          <div className={s.response}>
            {!response || (response && response.attempts < 50) ? (
              <>
                ...
                <div
                  className={`${s.button} ${answered ? s.disabled : ""}`}
                  onClick={submit}
                >
                  Answer
                </div>
            ...
            {loading ? <ClipLoader color="#3489db" /> : null}
            {errorText !== "" ? <p id={s.error}>{errorText}</p> : null}
            {answered ? <p id={s.success}>{successText}</p> : null}
          </div>
